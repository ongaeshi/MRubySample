...
class Player
  attr_accessor :x, :y, :w, :h, :vy, :speed, :tex
...
class FallingObject
  attr_accessor :x, :y, :w, :h, :vy, :type, :tex, :active, :on_ground, :lifetime

  def initialize(type, tex)
    @type = type
    @active = true
    @on_ground = false
    @lifetime = 0
...
def main
  Raylib.init_window(800, 600, "Jump Action with Texture - MRubySample")
  ...
  while !Raylib.window_should_close()
    if state == :playing
      ...
      if spawn_timer > interval
        if rand(100) < 40
          objects << FallingObject.new(:item, food_tex)
        else
          objects << FallingObject.new(:obstacle, obstacle_tex)
        end
        spawn_timer = 0
      end
