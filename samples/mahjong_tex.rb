class Tile
  ...
  def self.load_textures(dir_path)
    ["Man", "Pin", "Sou"].each do |s|
      (1..9).each do |v|
        @@textures["#{s.downcase[0]}#{v}"] = Raylib::Texture.new("#{dir_path}/#{s}#{v}.png")
      end
    end
    ["Ton", "Nan", "Shaa", "Pei"].each_with_index do |name, i|
      @@textures["z#{i+1}"] = Raylib::Texture.new("#{dir_path}/#{name}.png")
    end
    ["Haku", "Hatsu", "Chun"].each_with_index do |name, i|
      @@textures["z#{i+5}"] = Raylib::Texture.new("#{dir_path}/#{name}.png")
    end
    @@textures["back"] = Raylib::Texture.new("#{dir_path}/Back.png")
    @@textures["front"] = Raylib::Texture.new("#{dir_path}/Front.png")
  end

  def self.draw_back(x, y)
    tex = @@textures["back"]
    if tex
      tex.draw_resized(x, y, 40, 60, Raylib::WHITE)
    else
      Raylib.draw_rectangle(x, y, 40, 60, Raylib::GREEN)
      Raylib.draw_rectangle_lines(x, y, 40, 60, Raylib::BLACK)
    end
  end
  ...
  def draw(x, y, selected = false)
    ...
    tex = @@textures["#{@suit}#{@val}"]
    if tex
      front_tex = @@textures["front"]
      front_tex.draw_resized(x, y + y_offset, w, h, Raylib::WHITE) if front_tex
      tex.draw_resized(x, y + y_offset, w, h, Raylib::WHITE)
      if selected
        # 選択時のハイライト枠
        Raylib.draw_rectangle_lines(x, y + y_offset, w, h, Raylib::YELLOW)
        Raylib.draw_rectangle_lines(x+1, y + y_offset+1, w-2, h-2, Raylib::YELLOW)
      end
    else
      # テクスチャがない場合のフォールバック描画
      ...
      Raylib.draw_text(text, x + offset_x, y + y_offset + 20, 20, Raylib::BLACK)
    end
  end
end
...
def main
  Raylib.init_window(800, 600, "Mahjong Solo Texture - MRubySample")
  Raylib.set_target_fps(60)

  # テクスチャをロード
  Tile.load_textures("samples/mahjong_tex/tiles")
  ...
  while !Raylib.window_should_close()
    ...
    # 雀卓っぽい緑色背景
    Raylib.clear_background([34, 139, 34, 255]) # ForestGreen

    if state == :title
      Raylib.draw_text("Mahjong Solo Textured", 150, 200, 50, Raylib::RAYWHITE)
      Raylib.draw_text("Press SPACE to Start", 280, 300, 25, Raylib::RAYWHITE)
      
      # タイトル画面で裏面牌などを飾りに描画
      Tile.draw_back(300, 400)
      Tile.draw_back(350, 400)
      Tile.draw_back(400, 400)
      Tile.draw_back(450, 400)
    else
